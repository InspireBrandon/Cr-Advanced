            ...
            rowDataComputed() {
                ...
                if (self.searchTextGrid == "") {
                    ...
                    return self.rowData.filter(e => {
                        if(e.systemDepartment_Name == null) {
                            return (e.name.toUpperCase().indexOf(self.searchTextGrid.toUpperCase()) > -1);    
                        }
                        else {
                            return (e.name.toUpperCase().indexOf(self.searchTextGrid.toUpperCase()) > -1 || e.systemDepartment_Name.toUpperCase().indexOf(self.searchTextGrid.toUpperCase()) > -1);
                        }
                    })
                }
            }
        ...
        methods: {
            ...
            getImportDepartments() {
                ...
                Axios.get(process.env.VUE_APP_API +
                        `SuplierLocationImportTX/Departments?userID=${encoded_details.USER_ID}`)
                    .then(r => {
                        console.log(r.data)
                        self.rowData = r.data;
                    })
