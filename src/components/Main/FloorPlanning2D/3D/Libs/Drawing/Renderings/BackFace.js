...
class BackFace extends Boxbase {
    ...
    setPosition() {
        ...
        self.element.position.y = self.data.parent.element.position.y + ((0.5 * (self.data.parent.data.depth - self.data.depth - (self.data.parent.data.depth * 2)) + self.data.depth - self.data.offset.y) / self.pxlToMeterRatio); // (((parentY - ((self.data.depth) + self.data.y)) / self.pxlToMeterRatio) / 2);
        self.element.position.z = self.data.parent.element.position.z + ((0.5 * self.data.parent.data.height - 0.03) / self.pxlToMeterRatio);
