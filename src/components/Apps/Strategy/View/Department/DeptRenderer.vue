<template>
    <div style="display: flex;">
        <div style="height: 35px;" v-for="(item,idx) in params.context.componentParent.$parent.level" :key="idx"
        ...
        <v-tooltip right>
            <template v-slot:activator="{ on }">
                <div v-if="idx > params.context.componentParent.$parent.level - 1" v-on="on"
        ...
        methods: {
            ...
            getAllOthersToolTip() {
                ...
                self.params.data.ratioARR.forEach((element, idx) => {
                    if(idx > self.params.context.componentParent.$parent.level - 1)
